...
import { runOnJS, useAnimatedRef, useSharedValue, useAnimatedScrollHandler, withTiming } from 'react-native-reanimated';
...
function usePullRefreshScrollView(_ref) {
  ...
  const panGesture = Gesture.Pan().enabled(!refreshing).simultaneousWithExternalGesture(ref).onUpdate(e => {
